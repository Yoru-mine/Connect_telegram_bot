"""
Telegram-бот: заявки на канал + форма связи + мини-админка + соц. сети по кнопке.

ФУНКЦИИ
- Уведомления о заявках на вступление в канал (обоим админам), кнопки Принять/Отклонить,
  плюс "Принять все" одной командой.
- Форма связи: пользователи пишут боту -> сообщение с полной инфой о юзере уходит
  обоим админам -> админ отвечает через Reply -> ответ доставляется пользователю.
- Меню админа: все заявки / все сообщения / статистика / принять все /
  заблокировать-разблокировать / статус подписок.
- Меню пользователя: написать владельцу / другие соц. сети.
  Кнопка "другие соц. сети" САМА проверяет подписку пользователя на канал в момент нажатия
  и показывает ссылки только тем, кто подписан.
- Бот отслеживает подписки/отписки от канала в реальном времени (через chat_member апдейты)
  и хранит это в базе данных PostgreSQL (переживает деплои/рестарты, в отличие от файла на диске) —
  статус всегда актуальный, не протухает.
- Блокировка пользователей формы связи.
- Приветствие новым пользователям бота со ссылкой на канал.
- Встроенный health-check веб-сервер на $PORT — нужен для деплоя как Web Service на Render Free.

НАСТРОЙКА
1. pip install -r requirements.txt
2. Заведи бесплатную Postgres-базу, например на https://neon.tech, скопируй строку подключения.
3. Скопируй .env.example в .env и заполни все значения (токен, id админов, id канала,
   DATABASE_URL и т.д.). Файл .env НЕ коммитить в git — он уже в .gitignore.
4. Бот должен быть админом канала (право "Добавление участников" — для заявок,
   и просто быть админом — чтобы видеть события подписки/отписки).
5. CHANNEL_ID — числовой id канала (например -1001234567890), НЕ ссылка и не юзернейм,
   т.к. канал приватный. Как узнать: перешли любое сообщение ИЗ канала боту в личку —
   он покажет число, скопируй его в .env как CHANNEL_ID.
6. Запуск локально:  python join_requests_bot.py
7. Деплой на Render: см. README_DEPLOY.md рядом с этим файлом.

БЕЗОПАСНОСТЬ
- Никогда не публикуй .env и сам исходник, если в него случайно попал токен или DATABASE_URL.
- Если токен когда-либо был закоммичен в git или отправлен куда-то — считай его слитым
  и немедленно перевыпусти через @BotFather -> /revoke, затем впиши новый в .env.
"""
